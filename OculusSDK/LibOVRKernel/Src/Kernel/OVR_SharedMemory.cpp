...
static SharedMemoryInternal* DoFileMap(HANDLE hFileMapping, const char* fileName, bool openReadOnly, int minSize)
{
    ...
    if (!pFileView)
    {
        ...
        Logger.LogDebugF("FAILURE: Unable to map view of file for %s error code = %d", fileName, GetLastError());
        OVR_UNUSED(fileName);
        return NULL;
    }
...
static SharedMemoryInternal* AttemptOpenSharedMemory(const char* fileName, int minSize, bool openReadOnly)
{
    ...
    {
        Logger.LogTraceF("WARNING: Unable to open file mapping for %s error code = %d (not necessarily bad)", fileName, GetLastError());
        return NULL;
    }
...
static SharedMemoryInternal* AttemptCreateSharedMemory(const char* fileName, int minSize, bool openReadOnly, bool allowRemoteWrite)
{
    ...
    if (NULL == hFileMapping)
    {
        Logger.LogDebugF("FAILURE: Unable to create file mapping for %s error code = %d", fileName, GetLastError());
        return NULL;
    }
...
static SharedMemoryInternal* AttemptOpenSharedMemory(const char* fileName, int minSize, bool openReadOnly)
{
    ...
    if (hFileMapping < 0)
    {
        Logger.LogDebugF("WARNING: Unable to open file mapping for %s error code = %d (not necessarily bad)", fileName, errno);
        return NULL;
    }
...
static SharedMemoryInternal* AttemptCreateSharedMemory(const char* fileName, int minSize, bool openReadOnly, bool allowRemoteWrite)
{
    ...
    {
        Logger.LogDebugF("WARNING: Unable to unlink shared memory file %s error code = %d", fileName, errno);
    }
    flags |= O_EXCL;
#endif
    ...
    if (hFileMapping < 0)
    {
        Logger.LogDebugF("FAILURE: Unable to create file mapping for %s error code = %d", fileName, errno);
        return NULL;
    }
    ...
    {
        close(hFileMapping);
        Logger.LogDebugF("FAILURE: Unable to truncate file for %s to %d error code = %d", fileName, minSize, errno);
        return NULL;
    }
...
Ptr<SharedMemory> SharedMemoryFactory::Open(const SharedMemory::OpenParameters& params)
{
    ...
    if (Logger.GetMinimumOutputLevel() <= ovrlog::Level::Trace)
    {
        ...
        Logger.LogTraceF("%s shared memory region: %s > %d bytes",
