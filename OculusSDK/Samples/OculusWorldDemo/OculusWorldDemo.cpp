...
bool UseBarnacle = false;
bool IsBarnacleFirstUpdate = true;
Posef BarnaclePose;
Posef BarnacleStartInversePose;
...
void OculusWorldDemoApp::OnKey(OVR::KeyCode key, int chr, bool down, int modifiers)
{
    ...
    case Key_F5:
        if (!down)
        {
            UseBarnacle = !UseBarnacle;
            if (UseBarnacle) {
                IsBarnacleFirstUpdate = true;
                BarnaclePose = Posef::Identity();
                BarnacleStartInversePose = Posef::Identity();
            }
        }
        return;
    }
