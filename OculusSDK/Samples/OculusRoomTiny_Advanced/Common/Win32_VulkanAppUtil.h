...
#include <assert.h>
    ...
    // XXX For client-side interop, will move to Vk API implementation
    #include <d3d11.h>
    #include <dxgi.h>
...
// XXX change this to OVR_CAPI_VK.h when it exists...
#include "OVR_CAPI.h"
...
public:
    ...
    // XXX Temporary: clean up member COM pointers
    ...
    void DumpVkPhysicalDevice(ovrGraphicsLuid luid) const
    {
        ...
        Debug.Log("GPU " + std::string(gpuProps.properties.deviceName) + " (" + deviceType + ")");
        if (gpuDevID.deviceLUIDValid)
        {
            Debug.Log("LUID: " + dumpHex(gpuDevID.deviceLUID) + " ovrGraphicsLuid: " + dumpHex(luid));
        }
    ...
    VkPhysicalDevice ovrFindVkPhysicalDeviceForLuid(VkInstance instance, const ovrGraphicsLuid& luid)
    {
        VkPhysicalDevice gpus[10];
        uint32_t gpuCount = countof(gpus);

        CHECKVK(vkEnumeratePhysicalDevices(instance, &gpuCount, gpus));

        for (uint32_t i = 0; i < gpuCount; ++i)
        {
            VkPhysicalDeviceIDPropertiesKHX gpuDevID{ VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ID_PROPERTIES_KHX };
            VkPhysicalDeviceProperties2KHR gpuProps{ VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROPERTIES_2_KHR, &gpuDevID };
            // Note older AMD drivers crash here
            api.vkGetPhysicalDeviceProperties2KHR(gpus[i], &gpuProps);
            ...
            Debug.Log("Checking " + std::string(gpuProps.properties.deviceName) + " (" + deviceType + ")");

            if (gpuDevID.deviceLUIDValid)
            {
                if (memcmp(&luid, gpuDevID.deviceLUID, sizeof(gpuDevID.deviceLUID)) == 0)
                {
                    Debug.Log("Found Vulkan physical device " + std::string(gpuProps.properties.deviceName));
                    return gpus[i];
                }
            }
        }

        Debug.Log("Failed to find Vulkan physical device matching LUID " + dumpHex(luid));
        return nullptr;
    }
    ...
    // XXX This goes away once OVR_CAPI_Vk.h exists
    void CreateDxDevice(const ovrGraphicsLuid& luid)
    {
        ...
        hr = D3D11CreateDevice(Adapter, DriverType, 0, 0, 0, 0, D3D11_SDK_VERSION, &DxDevice, 0, &DxContext);
        Release(Adapter);
        Release(DXGIFactory);
        VALIDATE((hr == ERROR_SUCCESS), "D3D11CreateDevice failed");
    }

    bool InitDevice(const char* appName, int vpW, int vpH, const ovrGraphicsLuid& luid, bool windowed = true)
    {
        CreateDxDevice(luid);
        ...
        CHECK(physicalDevice = ovrFindVkPhysicalDeviceForLuid(instance, luid));

        DumpVkPhysicalDevice(luid);
        ...
        CHECK(drawCmd.Init(found.drawQueueFamilyIndex));
...
bool CmdBuffer::Init(uint32_t queueFamilyIndex)
{
    ...
    state = CmdBufferState::Initialized;
    return true;
}
...
bool CmdBuffer::Begin()
{
    ...
    state = CmdBufferState::Recording;
    return true;
}
...
bool CmdBuffer::End()
{
    ...
    state = CmdBufferState::Executable;
    return true;
}
...
bool CmdBuffer::Exec(VkQueue queue)
{
    ...
    state = CmdBufferState::Executing;
    return true;
}
...
public:
    ...
    void SetLayout(uint32_t mipLevel, VkImageLayout oldLayout, VkImageLayout newLayout,
        VkAccessFlagBits srcAccessMask, VkPipelineStageFlags srcStages, VkPipelineStageFlags dstStages)
    {
        static const std::unordered_map<VkImageLayout, VkAccessFlags> layoutAccessMask
        {
        ...
        imgBarrier.dstAccessMask = (layoutAccessMask.count(newLayout) > 0) ? layoutAccessMask.find(newLayout)->second : 0;
        ...
        vkCmdPipelineBarrier(Platform.drawCmd.buf, srcStages, dstStages, 0, 0, nullptr, 0, nullptr, 1, &imgBarrier);
    }
    ...
    bool Create(Style style, uint32_t width, uint32_t height)
    {
        ...
        SetLayout(0, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT,
            VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
        vkCmdBlitImage(Platform.drawCmd.buf, staging.img, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit, VK_FILTER_LINEAR);

        for (uint32_t mipLevel = 1; mipLevel < mipLevels; ++mipLevel)
        {
            // Switch to reading from the finer level...
            SetLayout(mipLevel - 1, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT,
                VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
            // ...and writing the coarser level
            SetLayout(mipLevel, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT,
                VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
            ...
            if (blit.dstOffsets[1].y < 1)
                blit.dstOffsets[1].y = 1;
            vkCmdBlitImage(Platform.drawCmd.buf, img, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit, VK_FILTER_LINEAR);
        }

        // Get all the mip levels ready for sampling
        for (uint32_t mipLevel = 0; mipLevel < mipLevels; ++mipLevel)
        {
            SetLayout(mipLevel, (mipLevel < (mipLevels - 1)) ? VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL : VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
public:
    ...
    {
        vkCmdBindDescriptorSets(Platform.drawCmd.buf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeLayout.pipeLayout, 0, 1, &descSet, 0, nullptr);
    }
...
public:
    ...
    bool Create(VkExtent2D size, const PipelineLayout& layout, const RenderPass& rp, const ShaderProgram& sp, const VertexBufferBase& vb)
    {
        ...
        // Flipped viewport so origin is bottom-left like GL
...
public:
    ...
    {
        Matrix4f mvp = proj * view * Matrix4f::Translation(pos);
        // Goddamnit
        mvp.Transpose();

        VkCommandBuffer cmdBuf = Platform.drawCmd.buf;
        vkCmdPushConstants(cmdBuf, pipeLayout.pipeLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(mvp), &mvp.M[0][0]);

        tex.Bind(pipeLayout);

        // Bind index and vertex buffers at the Model's byte offsets
        VkDeviceSize offset = firstIdx * sizeof(uint16_t);
        vkCmdBindIndexBuffer(cmdBuf, vb.idxBuf, offset, VK_INDEX_TYPE_UINT16);

        offset = vtxOffset * sizeof(Vertex);
        vkCmdBindVertexBuffers(cmdBuf, 0, 1, &vb.vtxBuf, &offset);
...
public:
    ...
    void Render(const Matrix4f& view, const Matrix4f& proj, const PipelineLayout& pipeLayout, const VertexBuffer<Vertex>& vb)
    {
        for (auto& model: models)
            model.Render(view, proj, pipeLayout, vb);
    }
